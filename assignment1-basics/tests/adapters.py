...
def run_linear(
    ...
    linear = LinearNobias(d_in, d_out)
...
def run_multihead_self_attention_with_rope(
    ...
    """
    from cs336_basics.attention import MultiheadAttention
    attention = MultiheadAttention(d_model, num_heads, max_seq_len, theta)
    state_dict = {"weight": q_proj_weight}
    attention.q_proj.load_state_dict(state_dict)
    state_dict = {"weight": k_proj_weight}
    attention.k_proj.load_state_dict(state_dict)
    ...
    Returns:
        tuple[dict[int, bytes], list[tuple[bytes, bytes]]]:
            ...
            merges:
                ...
                Merges are ordered by order of creation.
    """
    from cs336_basics.bpe_tokenizer import initialize_vocab, pre_tokenization, bpe_merges
    special_tokens_bytes = [special_token.encode("utf-8") for special_token in special_tokens]
    vocab = initialize_vocab(special_tokens_bytes)
    num_processes = 10
    pretokens_list = pre_tokenization(input_path, special_tokens_bytes, num_processes, True)
    vocab, merges = bpe_merges(vocab, vocab_size, pretokens_list)
    return vocab, merges
